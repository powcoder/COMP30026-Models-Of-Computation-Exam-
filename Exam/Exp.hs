...
printExp :: Exp -> String
printExp FALSE   = "false"
printExp TRUE    = "true"
printExp (VAR c) = [c]
printExp (NOT e)      = "(~"++ printExp e ++ ")"
printExp (AND  e1 e2) = "(" ++ printExp e1 ++ " & "   ++ printExp e2 ++ ")"
printExp (OR   e1 e2) = "(" ++ printExp e1 ++ " | "   ++ printExp e2 ++ ")"
printExp (XOR  e1 e2) = "(" ++ printExp e1 ++ " <+> " ++ printExp e2 ++ ")"
printExp (BIIM e1 e2) = "(" ++ printExp e1 ++ " <=> " ++ printExp e2 ++ ")"
printExp (IMPL e1 e2) = "(" ++ printExp e1 ++ " => "  ++ printExp e2 ++ ")"
...
data Token
  = FalseToken         -- Falsehood
  | TrueToken          -- Truth
  | Ident Char         -- Propositional variable
  | NegToken           -- Negation (~)
  | AndToken           -- Conjunction (&)
  | OrToken            -- Disjunction (|)
  | XorToken           -- Exclusive or (<+>)
  | BiimToken          -- Biimplication (<=>)
  | ImplToken          -- Conditional (=>)
...
scan ('f':'a':'l':'s':'e':s) = FalseToken : scan s
scan ('t':'r':'u':'e':s)     = TrueToken : scan s
...
pExp (FalseToken : tokens)
  = (FALSE, tokens)

pExp (TrueToken : tokens)
  = (TRUE, tokens)
